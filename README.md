...
**2. Поднять весь проект одной командой:**

```bash
make up
```

Эта команда собирает образы и поднимает все сервисы. При первом старте контейнер `app` автоматически:
дожидается готовности PostgreSQL → применяет миграции Alembic → создаёт администратора.

После запуска доступно:

| Сервис | URL | Назначение |
|---|---|---|
| Приложение (через Nginx) | http://localhost:8080 | основной вход |
| Приложение (напрямую) | http://localhost:8000 | отладка |
| Adminer | http://localhost:8081 | веб-клиент PostgreSQL |
...
make data      # заполнить БД демо-данными
make logs      # смотреть логи
make down      # остановить всё
make help      # список всех команд
```

> Для запуска **без Docker** скопируйте `.env.example` → `.env` и поднимите PostgreSQL/Redis локально.
...
Проект работает через Docker, поэтому команды выполняются в контейнерах.
Полный список — `make help`.

| Команда | Описание |
|---------|----------|
| `make up` | Поднять весь стек (сборка + запуск) |
| `make down` | Остановить и удалить контейнеры |
| `make ps` | Статус сервисов |
| `make logs` / `make logs-app` | Логи всех сервисов / только приложения |
| `make migrate` | Применить миграции Alembic |
| `make admin` | Создать администратора (admin@mail.ru / 12345) |
| `make seed` | Создать роли admin/manager/operator |
| `make data` | Заполнить БД демо-данными |
| `make test` | Запустить тесты (pytest) |
| `make shell` | Bash внутри контейнера app |
| `make docs` | Ссылка на Swagger (интерактивное API) |
| `make cache-keys` | Ключи кэша дашборда в Redis |
| `make cache-ttl` | Время жизни кэша (сек) |
| `make cache-clear` | Сбросить кэш дашборда |
| `make format` | Форматирование и линтинг (Ruff) |
...
## Мобильный API (/api/v1)

Отдельная версионированная группа маршрутов для мобильного клиента операторов с токен-аутентификацией (JWT, схема OAuth2 Bearer — stateless, подходит для слабой связи).

Интерактивная документация (Swagger): **http://localhost:8080/docs** — там есть кнопка **Authorize** для входа по email/паролю.

| Метод | URL | Описание |
|-------|-----|----------|
| POST | `/api/v1/auth/login` | Вход, выдача JWT-токена |
| GET | `/api/v1/auth/me` | Профиль по токену |
| GET | `/api/v1/wells` | Доступные скважины пользователя |
| POST | `/api/v1/reports` | Создание суточного рапорта с телефона |

Пример: вход возвращает токен, далее запросы идут с заголовком `Authorization: Bearer <token>`. POST рапорта возвращает 201 и рассчитанный объём нефти; дубль — 409, некорректные данные — 422, без токена — 401.

---

## Производительность

- **Кэш дашборда в Redis.** Тяжёлая агрегация по скважинам кэшируется на 1 час (`app/cache.py`) и сбрасывается при создании/удалении рапорта. Проверить: `make cache-keys`.
- **Eager Loading против N+1.** Списки рапортов подгружают скважину и компанию заранее (`selectinload`), поэтому число SQL-запросов не растёт с количеством строк (со 100 запросов до 3). Покрыто тестами `tests/test_performance.py`.
